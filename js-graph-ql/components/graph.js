export class Graph {
    ...
    createTooltip(data) {
        ...
        // Format the tooltip content with more information
        let content = `<strong>Date:</strong> ${data.date.toLocaleDateString()}<br>`;
        content += `<strong>Total XP:</strong> ${data.value.toLocaleString()}<br>`;

        // Add transaction amount if available
        if (data.amount) {
            content += `<strong>XP Earned:</strong> ${data.amount.toLocaleString()}<br>`;
        }

        // Add project path if available
        if (data.path) {
            content += `<strong>Project:</strong> ${data.path}`;
        }
